...
static void on_present(command_queue *, swapchain *swapchain, const rect *, const rect *, uint32_t, const rect *)
{
    ...
    // MODIFIED: Allow single draw calls to pass, as our overlay generates depth with exactly 1 draw call.
    if (queue_state.stats_per_used_depth_stencil.size() == 1 && queue_state.stats_per_used_depth_stencil.begin()->second.total.drawcalls == 0)
        return;
...
static void on_begin_render_effects(effect_runtime *runtime, command_list *cmd_list, resource_view, resource_view)
{
    ...
    {
        // MODIFIED: Allow 3 vertices to pass, as our overlay uses a fullscreen triangle (3 vertices) to generate depth.
...
extern "C" __declspec(dllexport) const char *NAME = "Fake Generic Depth";
